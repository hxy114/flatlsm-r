...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CompactionL0::FinalizeInputInfo(Version* _input_version) {
  input_version_ = _input_version;
  cfd_ = input_version_->cfd();
...
void CompactionL0::GetBoundaryKeys(
    VersionStorageInfo* vstorage,
    const std::vector<CompactionInputFiles>& inputs, Slice* smallest_user_key,
    Slice* largest_user_key, int exclude_level) {
  bool initialized = false;
...
void CompactionL0::GetBoundaryInternalKeys(
    VersionStorageInfo* vstorage,
    const std::vector<CompactionInputFiles>& inputs, InternalKey* smallest_key,
    InternalKey* largest_key, int exclude_level) {
  bool initialized = false;
...
std::vector<CompactionInputFiles> CompactionL0::PopulateWithAtomicBoundaries(
    VersionStorageInfo* vstorage, std::vector<CompactionInputFiles> inputs) {
  const Comparator* ucmp = vstorage->InternalComparator()->user_comparator();
  for (size_t i = 0; i < inputs.size(); i++) {
    if (inputs[i].level == 0 || inputs[i].files.empty()) {
...
// helper function to determine if compaction is creating files at the
// bottommost level
bool CompactionL0::IsBottommostLevel(
    int output_level, VersionStorageInfo* vstorage,
    const std::vector<CompactionInputFiles>& inputs) {
  int output_l0_idx;
  if (output_level == 0) {
...
// test function to validate the functionality of IsBottommostLevel()
// function -- determines if compaction with inputs and storage is bottommost
bool CompactionL0::TEST_IsBottommostLevel(
    int output_level, VersionStorageInfo* vstorage,
    const std::vector<CompactionInputFiles>& inputs) {
  return IsBottommostLevel(output_level, vstorage, inputs);
}

bool CompactionL0::IsFullCompaction(
    VersionStorageInfo* vstorage,
    const std::vector<CompactionInputFiles>& inputs) {
  size_t num_files_in_compaction = 0;
  size_t total_num_files = 0;
...
Status CompactionL0::InitInputTableProperties() {
  if (!input_table_properties_.empty()) {
    return Status::OK();
  }
  ...
  const ReadOptions read_options(Env::IOActivity::kCompaction);
  assert(input_version_);
  for (size_t i = 1; i < num_input_levels(); ++i) {
    for (const FileMetaData* fmd : *(this->inputs(i))) {
      std::shared_ptr<const TableProperties> tp;
      std::string file_name =
          TableFileName(immutable_options_.cf_paths, fmd->fd.GetNumber(),
...
CompactionL0::CompactionL0(
    ...
    const MutableCFOptions& _mutable_cf_options,
    const MutableDBOptions& _mutable_db_options,
    MemTable *input_memtable, size_t input_memtable_size,
    ...
    : input_vstorage_(vstorage),
      ...
      deletion_compaction_(_deletion_compaction),
      l0_files_might_overlap_(l0_files_might_overlap),
      input_memtables_(input_memtable),
      input_memtables_size_(input_memtable_size),
      inputs_(PopulateWithAtomicBoundaries(vstorage, std::move(_inputs))),
      grandparents_(std::move(_grandparents)),
      score_(_score),
      bottommost_level_(
      ...
      end_key_(end_key),
      smallest_user_key_(start_key_),
      largest_user_key_(end_key_),
      compaction_reason_(_compaction_reason),
      notify_on_compaction_completion_(false),
      enable_blob_garbage_collection_(
          _blob_garbage_collection_policy == BlobGarbageCollectionPolicy::kForce
  ...
  // setup input_levels_
  {
    input_levels_.resize(num_input_levels()-1);
...
void CompactionL0::PopulatePenultimateLevelOutputRange() {
  if (!SupportsPerKeyPlacement()) {
    return;
  }
...
bool CompactionL0::SupportsPerKeyPlacement() const {
  return penultimate_level_ != kInvalidLevel;
}

int CompactionL0::GetPenultimateLevel() const { return penultimate_level_; }

// smallest_key and largest_key include timestamps if user-defined timestamp is
// enabled.
bool CompactionL0::OverlapPenultimateLevelOutputRange(
    const Slice& smallest_key, const Slice& largest_key) const {
  if (!SupportsPerKeyPlacement()) {
    return false;
  }
...
// key includes timestamp if user-defined timestamp is enabled.
bool CompactionL0::WithinPenultimateLevelOutputRange(
    const ParsedInternalKey& ikey) const {
  if (!SupportsPerKeyPlacement()) {
    return false;
  }
...
bool CompactionL0::InputCompressionMatchesOutput() const {
  int base_level = input_vstorage_->base_level();
  bool matches =
      (GetCompressionType(input_vstorage_, mutable_cf_options_, start_level_,
                          base_level) == output_compression_);
...
void CompactionL0::AddInputDeletions(VersionEdit* out_edit) {
  for (size_t which = 0; which < num_input_levels() -1; which++) {
    for (size_t i = 0; i < inputs_[which].size(); i++) {
      out_edit->DeleteFile(level(which+1), inputs_[which][i]->fd.GetNumber());
    }
  }
}

bool CompactionL0::KeyNotExistsBeyondOutputLevel(
    const Slice& user_key, std::vector<size_t>* level_ptrs) const {
  assert(input_version_ != nullptr);
  assert(level_ptrs != nullptr);
  assert(level_ptrs->size() == static_cast<size_t>(number_levels_));
...
bool CompactionL0::KeyRangeNotExistsBeyondOutputLevel(
    const Slice& begin_key, const Slice& end_key,
    std::vector<size_t>* level_ptrs) const {
  assert(input_version_ != nullptr);
  assert(level_ptrs != nullptr);
...
// Mark (or clear) each file that is being compacted
void CompactionL0::MarkFilesBeingCompacted(bool mark_as_compacted) {
  /*for (size_t i = 0; i < num_input_levels(); i++) {
    for (size_t j = 0; j < inputs_[i].size(); j++) {
      assert(mark_as_compacted ? !inputs_[i][j]->being_compacted
                               : inputs_[i][j]->being_compacted);
...
// If compacting 3 L0 files, 2 L3 files and 1 L4 file, and outputting to L5,
// print: "3@0 + 2@3 + 1@4 files to L5"
const char* CompactionL0::InputLevelSummary(
    InputLevelSummaryBuffer* scratch) const {
  int len = 0;
  //bool is_first = true;

  len += snprintf(scratch->buffer + len, sizeof(scratch->buffer) - len,
                  "%" ROCKSDB_PRIszt "@%d", input_memtables_size_,
                  0);
  if (inputs_.size() > 0) {
    len += snprintf(scratch->buffer + len, sizeof(scratch->buffer) - len, " + ");
    len = std::min(len, static_cast<int>(sizeof(scratch->buffer)));
    ...
    len += snprintf(scratch->buffer + len, sizeof(scratch->buffer) - len,
                    "%" ROCKSDB_PRIszt "@%d", inputs_[0].size(),
                    inputs_[0].level);
    len = std::min(len, static_cast<int>(sizeof(scratch->buffer)));
...
uint64_t CompactionL0::CalculateTotalInputSize() const {
  uint64_t size = input_memtables_->ApproximateNvmMemoryUsage()*input_memtables_size_;
...
void CompactionL0::ReleaseCompactionFiles(Status status) {
  MarkFilesBeingCompacted(false);
  cfd_->compactionl0_picker()->ReleaseCompactionFiles(this, status);
}

void CompactionL0::ResetNextCompactionIndex() {
  assert(input_version_ != nullptr);
  input_vstorage_->ResetNextCompactionIndex(start_level_);
}

void CompactionL0::Summary(char* output, int len) {
  int write =
      snprintf(output, len, "Base version %" PRIu64 " Base level %d, inputs: [",
               input_version_->GetVersionNumber(), start_level_);
  if (write < 0 || write >= len) {
  ...
  for (size_t level_iter = 0; level_iter < num_input_levels() -1; ++level_iter) {
    if (level_iter > 0) {
      write += snprintf(output + write, len - write, "], [");
      if (write < 0 || write >= len) {
        return;
...
uint64_t CompactionL0::OutputFilePreallocationSize() const {
  uint64_t preallocation_size = 0;
...
std::unique_ptr<CompactionFilter> CompactionL0::CreateCompactionFilter() const {
  if (!cfd_->ioptions()->compaction_filter_factory) {
    return nullptr;
  }
...
std::unique_ptr<SstPartitioner> CompactionL0::CreateSstPartitioner() const {
  if (!immutable_options_.sst_partitioner_factory) {
    return nullptr;
  }
...
bool CompactionL0::IsOutputLevelEmpty() const {
  return inputs_.back().level != output_level_ || inputs_.back().empty();
}

bool CompactionL0::ShouldFormSubcompactions() const {
  return false;
  if (cfd_ == nullptr) {
    return false;
  }
...
bool CompactionL0::DoesInputReferenceBlobFiles() const {
  assert(input_version_);
...
uint64_t CompactionL0::MinInputFileOldestAncesterTime(
    const InternalKey* start, const InternalKey* end) const {
  uint64_t min_oldest_ancester_time = std::numeric_limits<uint64_t>::max();
  const InternalKeyComparator& icmp =
      column_family_data()->internal_comparator();
...
uint64_t CompactionL0::MinInputFileEpochNumber() const {
  uint64_t min_epoch_number = std::numeric_limits<uint64_t>::max();
  for (const auto& inputs_per_level : inputs_) {
    for (const auto& file : inputs_per_level.files) {
      min_epoch_number = std::min(min_epoch_number, file->epoch_number);
...
int CompactionL0::EvaluatePenultimateLevel(
    const VersionStorageInfo* vstorage,
    const ImmutableOptions& immutable_options, const int start_level,
    const int output_level) {
  // TODO: currently per_key_placement feature only support level and universal
