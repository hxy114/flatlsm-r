...
CompactionL0Iterator::CompactionL0Iterator(
    ...
    bool must_count_input_entries, const CompactionL0* compaction,
    ...
    : CompactionL0Iterator(
          ...
          std::unique_ptr<CompactionL0Proxy>(
              compaction ? new RealCompactionL0(compaction) : nullptr),
...
CompactionL0Iterator::CompactionL0Iterator(
    ...
    bool enforce_single_del_contracts,
    const std::atomic<bool>& manual_compaction_canceled,
    std::unique_ptr<CompactionL0Proxy> compaction, bool must_count_input_entries,
    const CompactionFilter* compaction_filter,
    const std::atomic<bool>* shutting_down,
    const std::shared_ptr<Logger> info_log,
    const std::string* full_history_ts_low,
...
void CompactionL0Iterator::ResetRecordCounts() {
...
void CompactionL0Iterator::SeekToFirst() {
  NextFromInput();
  PrepareOutput();
}

void CompactionL0Iterator::Next() {
...
bool CompactionL0Iterator::InvokeFilterIfNeeded(bool* need_skip,
                                              Slice* skip_until) {
...
void CompactionL0Iterator::NextFromInput() {
  at_next_ = false;
  validity_info_.Invalidate();
...
bool CompactionL0Iterator::ExtractLargeValueIfNeededImpl() {
...
void CompactionL0Iterator::ExtractLargeValueIfNeeded() {
...
void CompactionL0Iterator::GarbageCollectBlobIfNeeded() {
...
void CompactionL0Iterator::DecideOutputLevel() {
...
void CompactionL0Iterator::PrepareOutput() {
  if (Valid()) {
...
inline SequenceNumber CompactionL0Iterator::findEarliestVisibleSnapshot(
...
uint64_t CompactionL0Iterator::ComputeBlobGarbageCollectionCutoffFileNumber(
    const CompactionL0Proxy* compaction) {
  if (!compaction) {
    return 0;
  }
...
std::unique_ptr<BlobFetcher> CompactionL0Iterator::CreateBlobFetcherIfNeeded(
    const CompactionL0Proxy* compaction) {
  if (!compaction) {
    return nullptr;
  }
...
CompactionL0Iterator::CreatePrefetchBufferCollectionIfNeeded(
    const CompactionL0Proxy* compaction) {
  if (!compaction) {
    return nullptr;
  }
