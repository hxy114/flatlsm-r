...
bool L0CompactionPicker::NeedsCompaction(
...
class L0CompactionBuilder {
public:
    L0CompactionBuilder(const std::string& cf_name,
    ...
    CompactionL0* PickCompaction();

    // Pick the initial files to compact to the next level. (or together
    // in Intra-L0 compactions)
    __attribute__((unused)) void SetupInitialFiles();

    // If the initial files are from L0 level, pick other L0
    // files if needed.
    __attribute__((unused)) bool SetupOtherL0FilesIfNeeded();
    ...
    __attribute__((unused)) bool SetupOtherInputsIfNeeded();

    CompactionL0* GetCompaction(std::string &statr_key, std::string &end_Key);
...
void L0CompactionBuilder::PickFileToCompact(
        const autovector<std::pair<int, FileMetaData*>>& level_files,
        CompactToNextLevel compact_to_next_level) {
...
void L0CompactionBuilder::SetupInitialFiles() {
...
bool L0CompactionBuilder::SetupOtherL0FilesIfNeeded() {
...
void L0CompactionBuilder::SetupOtherFilesWithRoundRobinExpansion() {
...
bool L0CompactionBuilder::SetupOtherInputsIfNeeded() {
...
CompactionL0* L0CompactionBuilder::PickCompaction() {
 ...
 // printf("top:%zu, high:%zu,low:%zu\n",top_queue_.capacity(),high_queue_.capacity(),low_queue_.capacity());
  assert(!mem_list_.empty());
  CompactionL0* c= nullptr;
  auto memtable = mem_list_.front();
  //std::vector<std::pair<std::string,std::string>>top_range;
  std::string startey=memtable->GetMinKey(),endkey=memtable->GetMaxKey();
  ...
  if(!is_cover) {
    start_memtable_ = memtable;
    memtable_size_ = 1;
    output_level_ = 1;
    output_level_inputs_.level = 1;
    output_level_inputs_.files=std::move(input);
    if (!output_level_inputs_.empty()) {
      compaction_inputs_.push_back(output_level_inputs_);
    }
...
CompactionL0* L0CompactionBuilder::GetCompaction(std::string &start_key, std::string &end_key) {
  ...
  auto c = new CompactionL0(
          ...
          /* trim_ts */ "", memtable_size_, false /* deletion_compaction */,
          true, compaction_reason_);

  // If it's level 0 compaction, make sure we don't execute any other level 0
  // compactions in parallel
  compaction_picker_->RegisterCompactionL0(c);
...
*/
uint32_t L0CompactionBuilder::GetPathId(
...
bool L0CompactionBuilder::TryPickL0TrivialMove() {
...
bool L0CompactionBuilder::TryExtendNonL0TrivialMove(int start_index,
...
bool L0CompactionBuilder::PickFileToCompact() {
...
bool L0CompactionBuilder::PickIntraL0Compaction() {
  start_level_inputs_.clear();
  const std::vector<FileMetaData*>& level_files =
...
bool L0CompactionBuilder::PickSizeBasedIntraL0Compaction() {
  assert(start_level_ == 0);
...
Compaction* L0CompactionPicker::PickCompaction(
...
CompactionL0* L0CompactionPicker::PickCompactionL0(const std::string& cf_name,
                                   const MutableCFOptions& mutable_cf_options,
                                   const MutableDBOptions& mutable_db_options,
                                   VersionStorageInfo* vstorage,
                                   LogBuffer* log_buffer,
                                   std::list<MemTable*>& mem_list){
  L0CompactionBuilder builder(cf_name, vstorage, this, log_buffer,
                              mutable_cf_options, ioptions_,
                              mutable_db_options,mem_list,icmp_);
