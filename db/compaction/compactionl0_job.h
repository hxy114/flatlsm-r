...
#include "db/compaction/compactionl0_iterator.h"
...
public:
    ...
    virtual ~CompactionL0Job();

    // no copy/move
    CompactionL0Job(CompactionL0Job&& job) = delete;
    CompactionL0Job(const CompactionL0Job& job) = delete;
    CompactionL0Job& operator=(const CompactionL0Job& job) = delete;
...
protected:
    ...
    // kv-pairs
    void ProcessKeyValueCompaction(SubcompactionL0State* sub_compact);

    CompactionL0State* compact_;
...
private:
    ...
    void NotifyOnSubcompactionBegin(SubcompactionL0State* sub_compact);

    void NotifyOnSubcompactionCompleted(SubcompactionL0State* sub_compact);
    ...
    uint64_t GetCompactionId(SubcompactionL0State* sub_compact) const;
...
// file.
struct CompactionL0ServiceInput {
    ...
    static Status Read(const std::string& data_str, CompactionL0ServiceInput* obj);
    Status Write(std::string* output);

    // Initialize a dummy ColumnFamilyDescriptor
    CompactionL0ServiceInput() : column_family("", ColumnFamilyOptions()) {}

#ifndef NDEBUG
    bool TEST_Equals(CompactionL0ServiceInput* other);
    bool TEST_Equals(CompactionL0ServiceInput* other, std::string* mismatch);
#endif  // NDEBUG
};

// CompactionServiceOutputFile is the metadata for the output SST file
struct CompactionL0ServiceOutputFile {
    ...
    CompactionL0ServiceOutputFile() = default;
    CompactionL0ServiceOutputFile(
...
struct CompactionL0ServiceResult {
    Status status;
    std::vector<CompactionL0ServiceOutputFile> output_files;
    int output_level;
    ...
    // serialization interface to read and write the object
    static Status Read(const std::string& data_str, CompactionL0ServiceResult* obj);
    Status Write(std::string* output);

#ifndef NDEBUG
    bool TEST_Equals(CompactionL0ServiceResult* other);
    bool TEST_Equals(CompactionL0ServiceResult* other, std::string* mismatch);
#endif  // NDEBUG
};
