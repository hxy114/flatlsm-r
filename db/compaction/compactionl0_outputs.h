...
class CompactionL0Outputs {
 public:
  ...
  CompactionL0Outputs() = delete;

  explicit CompactionL0Outputs(const CompactionL0* compaction,
 ...
 private:
  friend class SubcompactionL0State;
  ...
  bool ShouldStopBefore(const CompactionL0Iterator& c_iter);
...
struct OutputL0Iterator {
 public:
  explicit OutputL0Iterator(const std::vector<CompactionL0Outputs::OutputL0>& a,
                          const std::vector<CompactionL0Outputs::OutputL0>& b)
      : a_(a), b_(b) {
    within_a = !a_.empty();
    idx_ = 0;
  }

  OutputL0Iterator begin() { return *this; }

  OutputL0Iterator end() { return *this; }

  size_t size() { return a_.size() + b_.size(); }

  const CompactionL0Outputs::OutputL0& operator*() const {
    return within_a ? a_[idx_] : b_[idx_];
  }

  OutputL0Iterator& operator++() {
    idx_++;
  ...
  bool operator!=(const OutputL0Iterator& /*rhs*/) const {
    return within_a || idx_ < b_.size();
  }

 private:
  const std::vector<CompactionL0Outputs::OutputL0>& a_;
  const std::vector<CompactionL0Outputs::OutputL0>& b_;
  bool within_a;
  size_t idx_;
};
