...
class SequenceL0IterWrapper : public InternalIterator {
 public:
  SequenceL0IterWrapper(InternalIterator* iter, const Comparator* cmp,
...
class CompactionL0Iterator {
 public:
  // A wrapper around Compaction. Has a much smaller interface, only what
  // CompactionIterator uses. Tests can override it.
  class CompactionL0Proxy {
   public:
    virtual ~CompactionL0Proxy() = default;
    ...
    virtual const CompactionL0* real_compaction() const = 0;
  ...
  class RealCompactionL0 : public CompactionL0Proxy {
   public:
    explicit RealCompactionL0(const CompactionL0* compaction)
    ...
    const CompactionL0* real_compaction() const override { return compaction_; }
   ...
   private:
    const CompactionL0* compaction_;
  };
  ...
  CompactionL0Iterator(
      ...
      bool must_count_input_entries, const CompactionL0* compaction = nullptr,
  ...
  CompactionL0Iterator(
      ...
      bool enforce_single_del_contracts,
      const std::atomic<bool>& manual_compaction_canceled,
      std::unique_ptr<CompactionL0Proxy> compaction,
 ...
 private:
  ...
  CreatePrefetchBufferCollectionIfNeeded(const CompactionL0Proxy* compaction);

  SequenceL0IterWrapper input_;
  ...
  std::unique_ptr<CompactionL0Proxy> compaction_;
...
inline bool CompactionL0Iterator::DefinitelyInSnapshot(SequenceNumber seq,
...
inline bool CompactionL0Iterator::DefinitelyNotInSnapshot(
